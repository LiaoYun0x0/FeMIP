...
class SEDataset(Dataset):
    def __init__(self,data_file,size=(320,320),stride=8):
        ...
        with open(data_file, 'r') as f:
            self.train_data = f.readlines()[:1000]
    ...
    def __getitem__(self, index: int):
        ...
        opt_img = cv2.resize(opt_img, (400, 400))
        ...
        sar_img = cv2.resize(sar_img, (400, 400))
        #sar_img = cv2.cvtColor(sar_img,cv2.COLOR_GRAY2RGB)
        #sar_img = cv2.copyMakeBorder(sar_img,150,150,150,150,cv2.BORDER_CONSTANT,value=(0,0,0))
    ...
    def _generate_label(self,Mr,Mq, qc, rc, coor, drop_mask=True):
        ...
        #mask1 = (50<= refer_pts[0,:]) & (refer_pts[0, :] < 320-50) & (50 <= refer_pts[1, :]) & (refer_pts[1, :] < 320-50)
        #mask1 = (50-rc[0]<= refer_pts[0,:]) & (refer_pts[0, :] < 356-50-rc[0]) & (50-rc[1] <= refer_pts[1, :]) & (refer_pts[1, :] < 356-50-rc[1])
        #mask1 = (max(150-rc[0], 0)<= refer_pts[0,:]) & (refer_pts[0, :] < min(556-150-rc[0], 320)) & (max(150-rc[1],0) <= refer_pts[1, :]) & (refer_pts[1, :] < min(556-150-rc[1], 320))
...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(
        SEDataset("/home/ly/Documents/zkj/ROIs/se_train.txt",size=size),
        batch_size=4,
        shuffle=True,
        num_workers=0,
        pin_memory=True)
