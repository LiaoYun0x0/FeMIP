...
class RotateRocketDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        # dropout query
        label_matrix = self._generate_label(Mr,Mq, qc, rc, (int(x), int(y))) #400x400
...
def build_Rotate_Rocket(
        train_data_file,
        test_data_file,
        size,
        stride):
    train_data = RotateRocketDataset(
        train_data_file,
        size=(320, 320),
        stride=8,
        aug=True)
    test_data = RotateRocketDataset(
        test_data_file,
        size=(320, 320),
        stride=8,
        aug=False)
...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(
        RotateRocketDataset("/home/ly/data/rockets/rockets_subject3_data/rockets/stage/test/test1.txt",size=size, aug=True),
        batch_size=4,
        shuffle=True,
        num_workers=0,
        pin_memory=True)
    ...
    while 1:
        for sample in dataloader:
            ...
            cv2.imshow(f"images/match0.jpg",out0)
            cv2.imshow(f"images/match1.jpg",out1)
            cv2.waitKey()
            num = num+ 1
