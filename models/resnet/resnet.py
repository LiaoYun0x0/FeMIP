...
class ResNet(nn.Module):
    ...
    def extract_endpoints(self,x):
        ...
        upx2 = self.upconv3(x2)  # 512-d [1, 512, 40, 140]
        upx2 = self.skipconnect(upx2, x1)  # 640-d [1, 1024, 40, 40]
        upx2 = self.iconv3(upx2)  # 512-d [1, 512, 40, 40]
        upx = self.upconv2(upx2)  # 256-d [1, 256, 80, 80]
        upx0 = self.skipconnect(upx, conv_fine)

        x5 = self.conv_fine(upx0)  # fine_dim-d [1, 256, 160, 160]
...
if __name__ == "__main__":
    ...
    # image = cv2.imread("/home/ly/Pictures/panda.jpg")
    # # y = model(torch.from_numpy(np.array(image).transpose(2,0,1)[np.newaxis,...]).float())
